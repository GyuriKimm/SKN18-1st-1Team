...
<<<<<<< HEAD
    ...
    st.markdown("""
        ### 🚗전기차 / 전기차 충전소 비율에 대한 분석🔋
        1.    지역별 인프라 불균형
        - 경기·제주는 충전소당 전기차 수가 높고 강원은 현저히 낮아, 인프라 배분에 지역 간 심각한 불균형이 있음을 알 수 있습니다.
        - 일부는 과잉설치(강원), 일부는 과소설치(경기·제주)로 효율적 인프라 운용이 이뤄지지 않을 수 있습니다.
        2.    충전소 유휴/과부하 문제
        - 강원처럼 낮은 비율 지역에서는 충전소 이용률이 낮아 유휴설비가 발생, 유지·관리 부담이 커질 수 있습니다.
        - 반대로, 비율이 높은 지역에서는 충전소 과부하(동시 사용, 대기, 혼잡 등) 현상이 나타날 수 있습니다.
        3.    전기차 확산 저해 요인
        - 이용자들은 충전 인프라 부족을 경험한 지역(경기·제주 등)에서는 전기차 구매를 꺼릴 수 있고,
        - 이로 인해 전기차 확산 속도가 지역별로 크게 차이가 날 수 있습니다.
        4.    도심·비도심 서비스 격차
        - 서울처럼 대도시는 비율이 낮아서 인프라 여건이 좋은 반면, 외곽·관광지(제주)는 여전히 취약하여 도심과 비도심 간 서비스 접근성 격차가 심화될 수 있습니다.
        5.    계절적/특수 시기 수요 집중
        - 제주·강원 등은 관광 성수기(여름, 휴가철)와 비수기의 수요 편차가 커, 성수기에는 충전소 부족이 극심해질 수 있습니다.
        6.    충전소 운영 효율성 및 수익성 문제
        - 저비율 지역은 충전소 유지·관리·운영 수익성이 낮아 사업자의 지속적 투자 유인이 줄어들 수 있고,
        고비율 지역은 반대로 장비 고장‧노후화 및 운영상 과부하에 따른 고장이 잦아질 수 있습니다.
        7.    환경 친화 정책의 실효성 저하
        •    지역에 따라 전기차 인프라와 정책 지원이 달라, 전체적인 친환경 정책 효과가 저하될 수 있습니다.""")
=======
    st.markdown("""
        ### 🚗전기차 / 전기차 충전소 비율에 대한 분석🔋
        1.	지역별 인프라 불균형

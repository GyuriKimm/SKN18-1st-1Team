...
def create_table_if_not_exists():
    ...
    # Get region columns (all except first and last)
    region_cols = df.columns[1:-1]  # Exclude first (date) and last (sum)
...
def db_insert(data, region_cols, cleaned_region_cols):
    ...
    placeholders = ['%s'] * (len(cleaned_region_cols) + 2)  # +2 for date and sum
...
def get_region_data(region_cols, cleaned_region_cols):
    ...
    for _, row in df.iterrows():
        ...
        for region in region_cols:
            car_count = row[region]
            if pd.isna(car_count) or car_count == '':
                region_values.append(0)
            else:
                try:
                    region_values.append(int(car_count))
                except:
                    region_values.append(0)
        ...
        # Create data tuple: (date, region1_value, region2_value, ..., regionN_value, sum)
        data_tuple = (date_obj.date(),) + tuple(region_values) + (sum_value,)
        data.append(data_tuple)
        
        print(f"�� {date_obj.date()}: {dict(zip(region_cols, region_values))} -> Sum: {sum_value}")
